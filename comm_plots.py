...
ygg, = plt.plot(x_axis, np.cumsum(by_col_2000), '.-', markersize=12, label='Yggdrasil, p = \{1K, 2K, 4K\}', color='g')

#plt.plot(x_axis, np.cumsum(by_col_4000), '.-', label='Yggdrasil, p = 4000')
planet_1k, = plt.plot(x_axis, np.cumsum(by_row_1000), 'x-', markersize=12, label='MLlib v1.6, p = 1K', color='b')
planet_2k, = plt.plot(x_axis, np.cumsum(by_row_2000), 'x-', markersize=12, label='MLlib v1.6, p = 2K', color='orangered')
planet_4k, = plt.plot(x_axis, np.cumsum(by_row_4000), 'x-', markersize=12, label='MLlib v1.6, p = 4K', color='magenta')
...
plt.title('Yggdrasil vs. Spark MLlib: Communication Cost', y=1.04)
plt.yscale('log')
plt.grid(True)
plt.savefig('Communication.png', transparent=True, bbox_inches='tight', pad_inches=0.2, dpi=900)
